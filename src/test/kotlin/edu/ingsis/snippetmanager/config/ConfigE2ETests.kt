...
import edu.ingsis.snippetmanager.security.JwtTokenUtil
...
class ConfigE2ETests @Autowired constructor(
    ...
    @BeforeEach
    fun setUp() {
        jwtToken = Jwt.withTokenValue("mockedToken")
            .header("alg", "none")
    ...
    fun `should get formatting config for user`() {
        val formattingConfig = FormattingConfigDto(spaceBeforeColon = true, spaceAfterColon = false, spaceAroundAssignment = true, newLinesBeforePrintln = 1, indentSpaces = 4)
    ...
    fun `should set formatting config for user`() {
        val formattingConfig = FormattingConfigDto(spaceBeforeColon = false, spaceAfterColon = true, spaceAroundAssignment = false, newLinesBeforePrintln = 2, indentSpaces = 2)
